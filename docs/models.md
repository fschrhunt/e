...
- `effort` on a model object declares its reasoning levels, in cycle order —
  ...
  "xhigh"]`). Built-ins carry their own; a file entry without `effort` has
  no reasoning knob. Levels are the exact strings sent as `reasoning_effort`
  (or the dialect's equivalent), so they must match what the backend accepts —
